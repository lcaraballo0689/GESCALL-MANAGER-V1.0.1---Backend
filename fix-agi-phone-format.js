...
# Get the destination phone number from AGI variables
my $phone_number = $AGI{'extension'} || $AGI{'dnid'} || '';
$phone_number =~ s/\\D//g;  # Keep only digits

# Strip 57 prefix if present (Colombia country code)
if ($phone_number =~ /^57(\\d{10})$/) {
    $phone_number = $1;
    agi_log("Stripped 57 prefix -> $phone_number");
}

agi_log("START: Phone=$phone_number");

if (!$phone_number || length($phone_number) < 10) {
...
# Find campaign using phone_number
my $campaign_id = '';
...
if ($row && $row->{campaign_id}) {
    ...
    agi_log("No campaign found for phone $phone_number");
    $dbh->disconnect;
    exit(0);
}
...
echo "AGI Updated - Now strips 57 prefix"

# Quick test
echo ""
echo "--- Quick Test ---"
echo -e "agi_extension: 573196233749\\n" | perl /usr/share/asterisk/agi-bin/callerid_local_presence.agi 2>&1
`;

console.log('Fixing Phone Format Mismatch...\n');
